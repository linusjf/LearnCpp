...
//
{
  ...
  //
  cout << endl;
  cout << "  Minimum distances from node 0:" << endl;
  cout << endl;
  for (i = 0; i < NV; i++)
    cout << "  " << setw(2) << i << "  " << setw(2) << mind[i] << endl;
  //
  //  Free memory.
  //
  delete[] mind;
...
#pragma omp single
    {
      cout << endl;
      cout << "  P" << my_id << ": Parallel region begins with " << nth
           << " threads.";
      cout << endl;
      cout << endl;
    }
    cout << "  P" << my_id << ":  First=" << my_first << "  Last=" << my_last
         << endl;
    //
...
#pragma omp single
      ...
      //  graph. Some threads might have no unconnected nodes left.
      //
...
#pragma omp barrier
      ...
      //  early. OpenMP does not like to BREAK out of a parallel region, so
      //  we'll just have to let the iteration run to the end, while we avoid
      //  doing any more updates.
      //
...
#pragma omp single
      {
        if (mv != -1) {
          connected[mv] = true;
          cout << "  P" << my_id << ": Connecting node " << mv << endl;
          ;
        }
      }
      //
...
#pragma omp single
    {
      cout << endl;
      cout << "  P" << my_id << ": Exiting parallel region.";
      cout << endl;
    }
  }
